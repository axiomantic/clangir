...
- `unit_requires_cpp`, which decides whether a unit's Nim bindings need the C++ backend. The answer is read off the IR, never off the file extension: a `.h` declaring a class is C++ and a `.hpp` declaring only C functions is not. Each clause mirrors one `importcpp` emission site and the emitters now call the same predicates, so an emitter cannot begin emitting `importcpp` for a shape the build configuration still treats as C. A parametrized case asserts the predicate and the emitted pragma agree for one IR shape per emission site.
