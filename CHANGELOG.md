...
- `tests/test_workorder_execution.py`: compile-link-execute gates for the generated suites. A generated test file that merely parses proves almost nothing -- `compile()` succeeds identically whether the package it imports works or raises `NameError` on its first line. These gates build a real C library, scaffold against its header, and run the generated suite for real. The Nim gate compiles, links and executes, asserting Tier 1 passes, every stub fails, and a failing case blocks no later case; a companion test plants a wrong Tier 1 expectation and asserts the suite turns red, so the gate cannot pass over an assertion that does not assert. The Python gate is `xfail(strict=True)` pending the ctypes importability fixes in PR #78, and turns into a failure the moment they land.
