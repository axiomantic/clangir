...
- ctypes: an enum-typed struct member, union member, array element, function parameter or return type is emitted as the enum's underlying integer instead of its C spelling. The generated module was un-importable for any header declaring one, in whichever of two ways the parser backend chose. libclang keeps the tag spelling, so the member reached the file as `("m", enum Colour)` -- not valid Python at all, a `SyntaxError` raised before a single line of the module ran. tree-sitter reduces the same reference to `("m", Colour)`, which parses and then raises `NameError`: an enum binds no ctypes class of its own, and where an alias is emitted at all it renders into the typedefs section, which comes *after* the records that use it. Both are resolved at the point of use rather than by reordering the sections, so the result no longer depends on the section order or on which backend parsed the header, and it agrees with the answer `_typedef_to_ctypes` already gave for `typedef enum E E` -- one convention, not two. `Enum` records no underlying type, so the existing `ENUM_CTYPE` default (`ctypes.c_int`) is used rather than a new one being invented. The over-reach is guarded in both directions: a tag and an ordinary identifier are separate namespaces in C, so `enum Tone { ... };` beside `typedef struct { ... } Tone;` is legal and an unprefixed `Tone` is the *record*; resolving every bare enum tag would silently retype an eight-byte record as a four-byte integer, which is a wrong answer computed quietly rather than an import that fails.
- `tests/test_scaffold_runs.py`: two execution gates for the above, each parameterized over both parser backends. A real C library and a real C++ library are compiled, the packages are scaffolded against the same headers, imported, and used. The assertions are deliberately paired, because neither half is sufficient alone: `ctypes.sizeof` of every record is compared against what the C compiler itself laid out (a member of the wrong width still round-trips a small value, so a value check cannot see it), and every member is then passed through a real by-value call into C and read back (a matching size says nothing about which member the value landed in). Covered: the `enum E` tag spelling with no typedef, an enum typedef whose name differs from the enum's own tag, a tag-less enum typedef, a union member, an array of enum, an enum as a parameter and as a return type, and a C++ `enum class` member -- whose enumerators are scoped to the tag, so the member's type is the only place the enum is named. The C++ accessors are reached through a plain `ctypes.CDLL` rather than through the generated prototypes, because libclang drops an `extern "C"` function from the IR and binding the gate to those prototypes would prove nothing under one of the two backends. Every record in the C fixture is a tag-less typedef so that no record is ever spelled `struct X`: a record named by its tag in a function signature reaches the module as `argtypes = [struct Tagged]`, which is a separate unfixed defect in the same writer and would make these gates red for a reason they are not about.
...
[0.40.2]: https://github.com/axiomantic/headerkit/compare/v0.40.1...v0.40.2
