...
- ctypes: a type named by its **tag** is resolved to something Python can use, instead of being emitted as its C spelling. `type_to_ctypes` returned the name verbatim, so `enum E` and `struct Inner` -- two words each -- reached the generated module and it did not parse. This affected every position a type occupies: struct and union members, array elements, pointees, and both halves of a function signature. Enums resolve to `ENUM_CTYPE`, records to the class this writer emits, and both are resolved at the point of use rather than by reordering the output, so the result no longer depends on the emitted section order. tree-sitter drops the tag keyword and reaches the same names bare, so a header-local table of the spellings each header can resolve is threaded through the writer in a `_TypeTable`.
- ctypes: an enum whose width cannot be proven is **refused rather than resolved**. `ENUM_CTYPE` is `ctypes.c_int` and `Enum` records no underlying type, so resolving every enum to it emitted members of the wrong width that *imported cleanly*: `enum class Small : unsigned char` gave a record of 8 bytes where C++ laid out 2, and an enum with an enumerator past `INT32_MAX` gave 4 where C laid out 8. A scoped enum, an enumerator outside signed 32-bit range, and an enumerator the backend left unevaluated all now keep the C spelling, which fails loudly at import. The alias such an enum would otherwise bind is withheld for the same reason -- it is emitted before the records that use it, so leaving it in would have made the refusal silent again.
...
- ctypes: a chained enum typedef binds its name again. `typedef enum E E1; typedef E1 E2;` left `E2` bound to nothing and dropped it from `declared_binding_names`; the alias chain is now followed to a fixed point.
...
- `tests/test_scaffold_runs.py`: eleven execution gates for the above, each parameterized over both parser backends. Real C and C++ libraries are compiled, the packages are scaffolded, imported and called. Sizes are compared against the compiler's own `sizeof` and members cross the ABI by value, because a member of the wrong width imports cleanly and only the size or the value can see it. Negative controls cover the two directions of over-reach -- a tag and an ordinary identifier are separate namespaces in C, so `enum Tone` beside `typedef struct { ... } Tone` and `struct Gauge` beside `typedef unsigned char Gauge` must each keep meaning what C says. Three gates pin failures that stay loud rather than becoming silent: an enum or record reached through an `#include` that tree-sitter never read, a record whose class is emitted after the record using it, and an incomplete record with no class at all.
