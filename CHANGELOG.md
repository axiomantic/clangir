...
- `CtypesWriter`: a scaffolded `package` layout now defines the library object its own output annotates, so the generated package can be imported. `header_to_ctypes` emitted `_lib.func.argtypes = [...]` for every function and never emitted a binding for `_lib`, so `import <pkg>` raised `NameError: name '_lib' is not defined` on the first annotation line. Nothing caught it because every scaffolding test asserted on the *text* of the generated file, and the text was correct -- a test grepping for `_lib.func.argtypes` passes identically whether the module works or dies on the line it just matched. The consequence reached further than a failed import: the tripwire `AGENTS.md` §1 requires imports `_bindings`, so a package that cannot be imported has no working tripwire either, and the invariant was unenforced rather than merely unmet. The package's `_bindings.py` now defines `_LIBRARY_NAME`, a `_load_library()` that consults `<PKG>_LIBRARY` before `ctypes.util.find_library`, and binds `_lib` from it; each function is additionally bound to a module-level name after its `argtypes`/`restype` are set, so `from <pkg> import *` yields configured callables rather than nothing. `_load_library()` raises `OSError` rather than returning `None` when the library is absent, which is what makes the tripwire fail when the binary is missing instead of passing with nothing behind it. The single-file layout is deliberately unchanged: it renders a fragment that expects the caller to supply `_lib`, which is the contract `header_to_ctypes(header, lib_name="mylib")` already had, and the loader is emitted only for the new keyword-only `library` argument.
- `CtypesWriter`: a typedef whose underlying type is an enum now aliases a ctypes type instead of itself. `_typedef_to_ctypes` stripped the `struct `/`union `/`enum ` prefix and aliased the tag, which is correct for a record -- the class is emitted above -- but an enum reaches this writer as loose integer constants and binds no name, so `typedef enum { OFF, ON } Switch;` emitted `Switch = Switch` and raised `NameError` at import. Any `typedef enum { ... } Name;` produced an un-importable module. The alias is now `ctypes.c_int`, which is what a C enum is at the ABI boundary. An existing assertion in `test_integration/test_roundtrip_ctypes.py` pinned the broken text and is corrected.
- `CtypesWriter`: the generated tripwire and unit test are valid Python when the header declares more than one function. Both templates interpolated a multi-line assertion block into an indented `textwrap.dedent` literal, and `dedent` measures the string *after* interpolation: with two or more functions the block's own 4-space indent became the common prefix, the template kept the rest of its source indentation, and the emitted file failed to parse with `IndentationError: unexpected indent` on line 1. One function produced a single-line block and looked correct, which is why the defect survived. The templates now dedent against a one-line token and substitute the block afterwards. The same pattern remains in the cffi, Lua and Nim writers and is not addressed here.
- `NimWriter`: `importc` no longer names a C tag that the header does not declare. `typedef enum { MODE_X, MODE_Y } Mode;` declares no `enum Mode` tag, but the writer emitted `importc: "enum Mode"` unconditionally, and the generated C failed to compile with `cast to incomplete type 'enum Mode'`. The failure was invisible until something *used* the type: Nim emits an `importc` type into its C only when a declaration references it, so bindings for a tag-less enum compiled cleanly as long as nothing declared a variable of that type. The same held for a tag-less `typedef struct { ... } Rec;` and `importc: "struct Rec"`. The spelling is now chosen by `Enum.is_typedef` / `Struct.is_typedef` -- the discriminator `headerkit.writers.cffi._find_typedef_enum_pairs` already documents and the Cython writer already uses -- which is set exactly when a typedef of that name exists, making the bare name a valid C type spelling whether or not the tag also exists. A tagged `typedef enum Mode { ... } Mode;` and a plain `struct Rec { ... };` are unaffected in behaviour; the former's emitted spelling changes from `enum Mode` to `Mode`, both of which C accepts.

### Added

- `tests/test_scaffold_runs.py`: execution gates that consume scaffolded output rather than reading it. A real C library is compiled from a fixture header carrying two tag-less `typedef enum`s, a tag-less `typedef struct` with a bit-field and two functions; the scaffolder runs against that same header; and the result has to survive being used. The ctypes package must import in a fresh interpreter and `thing_add(2, 3)` must return 5 through the loaded library, its typedef'd enum names must resolve to ctypes types, its generated tripwire must itself run and pass, and with no library present the import must fail with an `OSError` naming the library -- the `AGENTS.md` §1 tripwire invariant, asserted rather than assumed. The Nim package's bindings must compile, link against the same C object and execute, with the test program declaring a variable of every generated type so Nim is forced to emit each `importc` type into its C. Each of the four defects above was reproduced as a red run of these gates before it was fixed.

- `header_to_ctypes` accepts a keyword-only `library` argument naming the native library the generated module should load. It defaults to `None`, which preserves the existing fragment output byte for byte.
