...
- `WriterOption.coerce`: a `set` given for a list-typed option is sorted rather than converted in iteration order, so a generated file does not change between runs and defeat regenerate-and-diff. `NimWriter` sorts the same way for `extra_context` values, which do not pass through coercion.
...
[0.43.0]: https://github.com/axiomantic/headerkit/compare/v0.42.2...v0.43.0
