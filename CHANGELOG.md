...
- `AllowedSkip` refuses a degenerate entry in `__post_init__`, at import time. `AllowedSkip(nodeid="*", reason="")` matched every skip in the suite and passed every check the class then made, because the empty string is a substring of every string and `fnmatch(x, "*")` is always true. For a developer facing a red skip, adding an entry was therefore strictly cheaper than fixing it, and nothing made the cheap path visibly wrong. A nodeid must now carry at least 20 literal characters, which refuses both `*` and `tests/*`; a reason at least three words; a justification at least fifteen. Each constraint has a test asserting the construction it was written against is rejected, and a negative control asserting a real entry still constructs.
