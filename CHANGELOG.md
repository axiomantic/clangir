...
- `ALLOWED_SKIPS` in `tests/skip_policy.py` is the narrow, per-entry exception, and is deliberately not a switch. An entry names one test *and* one reason, and both must match: a nodeid pattern alone would silently widen to cover the same test skipping later because a toolchain went missing. It ships with exactly one entry, for the padding-corpus records built entirely from unnamed bit-fields -- `test_every_member_occupies_the_same_bits_as_in_c` locates each *named* member's bits, so for those entries there is nothing to locate, and their layout is covered by the `sizeof`/alignment test in the same class, which does run. That skip is a property of the corpus entry and fires on a machine with every toolchain installed, which is the only admissible justification: "it is currently red in CI" is not one.
