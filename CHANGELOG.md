...
- `ctypes` writer: a packed record containing a bit-field no longer produces the wrong layout. `_pack_ = 1` pins the record's *alignment* to a byte, but it does not change how ctypes allocates a bit-field's storage unit: a field declared `unsigned int` still reserves a full 32-bit unit and the next member starts after it, where a packed record in C reserves only the bits the field declares. Measured against a compiled C probe, `struct __attribute__((packed)) { unsigned char a; unsigned int b : 8; unsigned char c; }` is 3 bytes with `c` at bit 16 in C, and the generated class was 6 bytes with `c` at bit 40. The defect was never padding-specific -- it moved named bit-fields, anonymous padding bit-fields, and every member following them. A mixed record measured 12 bytes against C's 9, with three of its five members displaced.
  ...
  Shapes that remain inexpressible are reported instead of emitted silently, and the detection is now a layout model rather than a single-condition guess. The writer walks C's packed offset -- which never rounds a bit-field up to its declared storage unit -- alongside the offset ctypes will produce from the emitted spelling, and names the first field where the two disagree. The previous condition read the *unpacked* running offset, which diverges from packed truth as soon as any bit-field crosses its declared unit, and so missed shapes silently: `struct __attribute__((packed)) { unsigned short a : 12; unsigned char b : 4; unsigned char c; }` placed `b` at bit offset 12 inside a one-byte carrier, where every write to `S.b` was discarded and every read returned 0, with no marker. Measured against a compiled C probe across eight packed bit-field shapes on CPython 3.10, 3.13 and 3.14, the five that ctypes cannot reproduce are all flagged and the three it can are all left clean.

  Padding in a packed record is now respelled from the packed offset rather than the unpacked one, so an unnamed bit-field reserves exactly the width it declares instead of the span its declared storage unit would have covered.
...
- `cffi` writer: a packed record emitted a bare `/* packed */` comment, which reads as a marker but has no effect -- a C comment cannot change layout. Measured with cffi, that declaration comes back at 12 bytes where C measures 6, and nothing reports the discrepancy. cffi's `cdef` parser rejects `__attribute__((packed))` in either position, so the attribute cannot simply be emitted; its only packing mechanism is `ffi.cdef(..., packed=True)`, which applies to every record in the call rather than to one. The comment now names that argument and warns that a header mixing packed and unpacked records needs them split across separate `cdef()` calls, so a wrong layout can no longer be obtained in silence.
  ...
  - Generated classes that set `_pack_` emit a `DeprecationWarning` on Python 3.14: `_pack_` implies MSVC layout, and the implicit default is slated to become an error in Python 3.19. `_pack_` is rejected outright in combination with `_layout_ = "gcc-sysv"`. The measured layouts above are correct under the current default on every supported version; pinning `_layout_` explicitly is left to a separate change because it needs a Python 3.10 fallback.
