...
- `NimWriter`: the `std::` probes gating the `CppString`, `UniquePtr` and `SharedPtr` helper declarations now match what a backend actually produces. They tested for the substring `std::string`, but libclang reports such a field as the bare name `string`, so every probe was dead and a header using `std::string` generated bindings referencing an undeclared `CppString`. Matching is on the unqualified head, and a C tag is still distinguished without ambiguity because it keeps its `struct `/`union `/`enum ` keyword -- a C header declaring `struct vector` is not mistaken for `std::vector`.
...
- `NimWriter`: a C++ unit that binds nothing linkable reports *skipped* instead of passing. Such a unit -- one binding only templates -- has no symbol to link, since a generic emits none until it is instantiated, and no complete class to size. The fallback asserted `check declared(<pkg>)`, true by construction, under a test named "every bound entry point compiles and links". Measured with no native library built and no `-l` flag at all, that printed `[OK] every bound entry point compiles and links`: a pass stating the opposite of what it checked, which is the green mirage `AGENTS.md` forbids outright. The generated file now names the reason and skips, and a gate asserts it does not report success in that state.

- The generated `nim.cfg` quotes include and library paths, so one containing a space survives. Nim strips the outer quotes from a config value and word-splits the result, so `--passC:"-I/opt/na me"` reached clang as `-I/opt/na` and `me` and failed with `no such file or directory: 'me'`. Paths are emitted as `-I'<dir>'`, whose inner quotes survive that split; a path containing a single quote cannot be expressed in the format at all and is refused with a message saying so, rather than emitted in a form that means something else. Include directories are also compared after resolution, so a relatively-passed directory no longer duplicates the header's own `-I`.
...
[Unreleased]: https://github.com/axiomantic/headerkit/compare/v0.43.0...HEAD
[0.43.0]: https://github.com/axiomantic/headerkit/compare/v0.42.0...v0.43.0
