...
- `cffi` writer: a packed record emitted a bare `/* packed */` comment, which reads as a marker but has no effect -- a C comment cannot change layout. Measured with cffi, that declaration comes back at 12 bytes where C measures 6, and nothing reports the discrepancy. cffi's `cdef` parser rejects `__attribute__((packed))` in either position, so the attribute cannot simply be emitted; its only packing mechanism is `ffi.cdef(..., packed=True)`, which applies to every record in the call rather than to one. The comment now names that argument and warns that a header mixing packed and unpacked records needs them split across separate `cdef()` calls, so a wrong layout can no longer be obtained in silence.
  ...
  - `_pack_` **changes the layout** on Python 3.14, it does not merely warn. From 3.14 `_pack_` selects the MSVC memory layout, where earlier versions used the System V rules, so one generated module is laid out two different ways depending on the interpreter that imports it. Measured: `struct __attribute__((packed)) { unsigned char a : 8; unsigned int b : 20; }` is 4 bytes in C and on CPython 3.10 and 3.13, and 5 bytes on 3.14, from the same generated class. Of 1792 swept shapes, 538 are reproduced exactly on 3.10 and 3.13 against 343 on 3.14; the rest are reported. The check runs on the interpreter that generates the module, so generate on the interpreter you will run. `_pack_` is rejected outright in combination with `_layout_ = "gcc-sysv"`, so pinning the layout is not a drop-in change and is left to a separate one.
